...
import {CATEGORIES} from '@/components/Drawer/Sidebar';
  ...
  try {
    ...
    for (const category of CATEGORIES) {
      ...
      if (category === EMPTY_CATEGORY) {
        console.log(
          `Skipping category: ${category} (intentionally left empty)`
        );
        continue;
      }
    ...
    return res.status(200).json({
      ...
      categories: CATEGORIES.filter((category) => category !== EMPTY_CATEGORY)
    });
  } catch (error) {
    console.error('Error populating database:', error);
    return res.status(500).json({error: 'Failed to populate database'});
  }
}
